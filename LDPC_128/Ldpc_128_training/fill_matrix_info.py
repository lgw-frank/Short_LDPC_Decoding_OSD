...
class Code:
    ...
    # numbering each edge in H with a unique number whether horizontally or vertically       
        ...
        #extending parity check matrix by adding redundant rows for 'ENMS'
        if GL.get_map('selected_decoder_type') == 'ENMS':
            self.extending_check_matrix()                    

    def extending_check_matrix(self):
        self.expanded_H = self.H.tolist()
        m = self.check_matrix_row
        self.extra_rows = []
        min_sum = self.max_chk_degree*2-4
        for i in range(m-1):
            for j in range(i+1,m):
                tmp = (self.H[i]+self.H[j])%2
                non_zero_count = tmp.sum()
                if non_zero_count <= min_sum:
                  added_row = (self.H[i]+self.H[j])%2
                  self.expanded_H.append(added_row)              
        #update  the redundant check matrix and related info
        self.check_matrix_row = len(self.expanded_H)
        self.H = np.array(self.expanded_H)
        self.extra_rows = self.check_matrix_row - m 
